...
Description: This is a private plugin to remap some URLs hard-coded into old posts in SCOTXblog to new locations that will not break if the old locations change in the TAMES switch.
Version: 0.5
Author: Don Cruse
...
function tames_remap_electronic_briefs($content) {
    ...
    $content=preg_replace($ebrief_formats,'href="https://s3.amazonaws.com/docketdb/scotx/briefs/',$content);
    return $content;
}
...
$content=preg_replace($html_orders_format,"href=\"https://data.scotxblog.com/scotx/orders/$1$2-$4-$5.htm",$content);
        return $content;
}
...
$content=preg_replace($old_docketdb,"href=\"http://data.scotxblog.com/scotx/no/$4",$content);
	return $content;
}

add_filter( 'the_content', 'tames_remap_docketdb_docket_pages', 34 );
add_filter( 'the_content_feed', 'tames_remap_docketdb_docket_pages', 34 );
